...
func main() {
	...
	// 2. Connect Postgres and run migrations.
	...
	// 10. Context for background workers.
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()

	// 11. Start analysis consumer in a goroutine.
	if analysisSvc != nil {
		go func() {
			if err := analysisSvc.Run(ctx, cfg.Kafka.AnalysisTopic); err != nil {
				slog.Error("analysis service stopped", slog.String("error", err.Error()))
			}
		}()
	}
