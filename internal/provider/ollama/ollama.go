...
// New creates an Analyzer for the Ollama provider.
...
// Analyze sends an error event to Ollama and returns a structured AnalysisResult.
func (a *Analyzer) Analyze(ctx context.Context, event domain.AnalysisEvent) (domain.AnalysisResult, error) {
	...
	for attempt := 0; attempt < maxRetries; attempt++ {
		...
		if err != nil {
			return result, fmt.Errorf("ollama: create request: %w", err)
		}
		...
		if status == http.StatusTooManyRequests || status >= 500 {
			lastErr = fmt.Errorf("ollama: status %d", status)
			continue
		}

		if status != http.StatusOK {
			return result, fmt.Errorf("ollama: status %d: %s", status, string(respBytes))
		}

		var ollamaResp ollamaResponse
		if err := json.Unmarshal(respBytes, &ollamaResp); err != nil {
			return result, fmt.Errorf("ollama: unmarshal response: %w", err)
		}

		return parseResult(ollamaResp.Response)
	}

	return result, fmt.Errorf("ollama: max retries exceeded: %w", lastErr)
}
...
func parseResult(text string) (domain.AnalysisResult, error) {
	...
	if err := json.Unmarshal([]byte(text), &aj); err != nil {
		return domain.AnalysisResult{}, fmt.Errorf("ollama: parse result JSON: %w", err)
	}
